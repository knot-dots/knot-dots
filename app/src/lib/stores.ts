...
export const containerDialogState = writable<ContainerDialogState | undefined>();
...
export const newContainer: Writable<NewContainer | undefined> = {
	subscribe(run, invalidate) {
		return containerDialogState.subscribe((state) => run(state?.container), invalidate);
	},
	set(container) {
		containerDialogState.set(container ? { kind: 'create', container } : undefined);
	},
	update(updater) {
		containerDialogState.update((state) => {
...
export function openContainerCopyDialog(container: NewContainer, request: PendingContainerCopy) {
	containerDialogState.set({ kind: 'copy', container, request });
}
