<script lang="ts">
	...
	interface Props {
		editable?: boolean;
		value: { type: string; url: string } | undefined;
	}
		...
		if (input.files instanceof FileList && input.files.length > 0) {
			try {
				...
				value = {
					...
					url: await uploadAsFormData(input.files[0])
				};
...
{#if editable}
	...
	<div>
		<span class="value value--is-editable">
			...
			{:else if value}
				<img alt={label} src={transformFileURL(value.url)} />
				<button
		...
		<p class="help">{$_('custom_favicon_help')}</p>
	</div>
{:else}
