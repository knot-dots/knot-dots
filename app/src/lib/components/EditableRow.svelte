<script lang="ts">
	...
	// Ensure unit is initialized safely outside of template expressions
	$effect(() => {
		if ('unit' in container.payload && (container.payload.unit == null || container.payload.unit === undefined)) {
			container.payload.unit = '' as any;
		}
	});
</script>
