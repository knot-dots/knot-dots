...
function prepareOrderByExpression(sort: string) {
	let order_by = sql.fragment`(c.payload->>'title') COLLATE human_sort, (c.payload->>'name') COLLATE human_sort, c.guid`;
	if (sort == 'modified') {
...
export function getManyOrganizationalUnitContainers(filters: {
	...
	return async (
		...
		if (filters.include?.terms) {
			conditions.push(
				sql.fragment`to_tsquery('german', ${filters.include.terms
					.trim()
					.split(' ')
					.map((t) => `${t}:*`)
					.join(' & ')}) @@ jsonb_to_tsvector('german', c.payload, '["string", "numeric"]')`
...
export function getAdministrativeAreasByName(name: string) {
	return async (connection: DatabaseConnection) => {
		return connection.any(sql.type(administrativeArea)`
