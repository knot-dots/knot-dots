...
export default (async function load({ depends, locals, parent, url }: LoadInput) {
  ...
  const { currentOrganization, currentOrganizationalUnit, defaultOrganizationGuid } = (await parent()) as ParentData;
  const features = createFeatureDecisions(locals.features);

  const organizationScope = Array.from(
    new Set([currentOrganization.guid, defaultOrganizationGuid].filter((guid): guid is string => Boolean(guid)))
  );
  ...
  if (currentOrganizationalUnit) {
    ...
    subordinateOrganizationalUnits = relatedOrganizationalUnits
      .filter((unit: OrganizationalUnitContainer) => unit.payload.level > currentOrganizationalUnit.payload.level)
  ...
  return { containers: filtered, facets, facetLabels: categoryContext?.labels, categoryOptions: categoryContext?.options ?? null };
});
