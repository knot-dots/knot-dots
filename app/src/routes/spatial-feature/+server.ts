...
import { z } from 'zod';
import { getAdministrativeAreaByGeometry, getAdministrativeAreasByName } from '$lib/server/db';
...
export const GET = (async ({ locals, url }) => {
	...
	if (params.data.name) {
		return json(await locals.pool.connect(getAdministrativeAreasByName(params.data.name)));
	}
