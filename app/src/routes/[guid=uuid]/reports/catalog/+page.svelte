<script lang="ts">
	...
	const list = createPaginatedList<Container<ReportPayload>>({
		fetchPage: async ({ offset, signal }) => {
			const result = await fetchContainerPage<Container<ReportPayload>>({
		...
		initialItems: () => data.containers as Container<ReportPayload>[],
