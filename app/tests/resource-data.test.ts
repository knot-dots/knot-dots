...
test.describe('Resource Data Collections in Goals', () => {
	...
	test('add Budget section to goal', async ({ dotsBoard, isMobile, testGoal }) => {
		...
		// Verify section heading is visible
		await expect(section.getByRole('heading', { level: 2 })).toHaveText('Budget');

		// Clean up - delete the section
		await dotsBoard.overlay.deleteSection(section);
	});

	test('create resource data item in goal budget section', async ({
		dotsBoard,
		isMobile,
		testGoal,
		testResourceV2
		...
		// Navigate back to the goal overlay
		await dotsBoard.overlay.backButton.click();
	...
	test('add entries to goal budget resource data table', async ({
		dotsBoard,
		isMobile,
		testGoal,
		testResourceV2
		...
		// The total should now show 25,000 (or similar formatted)
		await expect(section.locator('.resource-data-card__amount')).toContainText('25,000');
